...
const getCar = arr => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].id === 33) {
      result.push(arr[i].car_year);
      result.push(arr[i].car_make);
      result.push(arr[i].car_model);
    }
  }
  return result;
};

console.log(getCar(inventory));
...
console.log(carSort(inventory));

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
const carYears = function(arr) {
  let carYears = [];
  for (let i = 0; i < arr.length; i++) {
